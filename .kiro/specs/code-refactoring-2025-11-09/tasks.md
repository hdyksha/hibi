...
- [ ] 6.2 TodoItem メインコンポーネントのリファクタリング
...
- [ ] 8.2 Filter メインコンポーネントのリファクタリング
  - `Filter.tsx` をサブコンポーネントを使用するように書き換え
  - 共通UIコンポーネント（Button、Input など）を使用
  - _要件: 2.1, 2.2, 5.1_

- [ ] 8.3 Filter コンポーネントのテスト更新
  - 既存のテストが引き続き合格することを確認
  - 必要に応じてテストを更新（シンプルなテストに留める）
  - _要件: 9.1, 9.2_
...
- [ ] 9.2 Archive メインコンポーネントのリファクタリング
  - `Archive.tsx` をサブコンポーネントを使用するように書き換え
  - 共通UIコンポーネント（Button など）を使用
  - _要件: 2.1, 2.2, 5.1_

- [ ] 9.3 Archive コンポーネントのテスト更新
  - 既存のテストが引き続き合格することを確認
  - 必要に応じてテストを更新（シンプルなテストに留める）
  - _要件: 9.1, 9.2_
...
- [ ] 13.1 サーバーサイドの不要なコード削除
  - `server/src` 配下のファイルを確認
  - 使用されていない関数、変数、インポートを削除
  - コメントアウトされたコードを削除
  - _要件: 4.1, 4.2, 4.3, 4.4_

- [ ] 13.2 クライアントサイドの不要なコード削除
  - `client/src` 配下のファイルを確認
  - 使用されていない関数、変数、インポートを削除
  - コメントアウトされたコードを削除
  - _要件: 4.1, 4.2, 4.3, 4.4_
...
- [ ] 14.1 サーバーサイドのドキュメント追加
  - 複雑なロジックにコメントを追加
  - 公開APIにJSDocコメントを追加
  - 新しいユーティリティ関数に使用例を追加
  - _要件: 13.1, 13.2, 13.3_

- [ ] 14.2 クライアントサイドのドキュメント追加
  - 複雑なロジックにコメントを追加
  - 公開APIにJSDocコメントを追加
